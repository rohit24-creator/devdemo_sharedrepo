...
export default function TripExpenseListingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data } = await api.get("/trips/tripExpenseData.json");
  ...
  const handleActionClick = (action, row) => {
    if (action === "delete") {
      ...
      console.log("Edit trip expense", row);
    } else if (action === "view") {
      console.log("View trip expense", row);
    } 
  };
  ...
  const secondIconMenu = [
    { label: "+ Add New", onClick: () => router.push(TRIP_ROUTES.tripExpense) },
  ];

...
export default function ContainersListPage() {
  ...
  const secondIconMenu = [
    { label: "+ Add New", onClick: () => router.push(BOOKING_ROUTES.containers) },
  ];
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const { data } = await api.get("/bookings/containers.json");
  ...
  const onSearch = (filters) => {
    ...
    if (filters.fromDate) {
      filtered = filtered.filter(row =>
        row.pickupTime && new Date(row.pickupTime) >= new Date(filters.fromDate)
      );
    }
    if (filters.toDate) {
      filtered = filtered.filter(row =>
        row.pickupTime && new Date(row.pickupTime) <= new Date(filters.toDate)
      );
    }

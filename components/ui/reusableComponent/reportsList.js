...
export default function ReportsList({
  ...
  const activeTab = useMemo(() => 
    hasTabs && typeof hasTabs === 'object' ? hasTabs.activeTab || Object.keys(tabData)[0] || "summary" : "summary",
    [hasTabs, tabData]
  );
  ...
  // Keep the original simple renderTable function
  const renderTable = useCallback((tableColumns, tableRows) => (
    <Card>
  ...
  ), [displayCount, setDisplayCount, currentPage, setCurrentPage, totalPages, isAllSelected, toggleSelectAll, selectedRows, toggleRow, showActions, effectiveActions, handleSort]);
  ...
  const mainContent = hasTabs && typeof hasTabs === 'object' ? (
    ...
    <div className="mt-3">{renderTable(currentColumns, currentRows)}</div>
  );

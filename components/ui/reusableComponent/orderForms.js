...
export function renderOrderFieldWithModals(
  ...
  const { name, label, type = "text", disabled = false, options = [], wide = false, placeholder, unitOptions, plusAction } = fieldConfig;
              ...
              <FormControl>
                {["companyCode", "branchCode"].includes(name) ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={plusAction} className="p-1 rounded border border-[#E7ECFD] bg-white hover:bg-gray-100">
...
export function OrdersForm({ sections = [], tableAccordion = true, disableAccordion = false }) {
  ...
  // If disableAccordion is true, render all sections open, no triggers
  if (disableAccordion) {
    return (
      <>
        {sections.map((section, index) => (
          <div key={index} className="mb-4 border rounded-md">
            <div className="bg-[#006397] text-white px-4 py-2 rounded-t-md font-semibold">
              {section.title}
            </div>
  ...
  // Default: normal accordion logic
  return (
    <>
